...
canvas.addEventListener('touchstart', (event) => {
    if (event.touches.length == 1) {
        ...
        touchesDistance = Math.hypot(t2.clientX - t1.clientX, t2.clientY - t1.clientY);
    }
}, { passive: false });
function startAction(x, y, b) {
...
function performZoom(scale, anchorX, anchorY) {
...
function splitLine(l, p) {
    intervals.splice(l, 1);
    let l2 = lines.splice(l, 1)[0];
    let l1 = l2.splice(0, p);
    l2.splice(0, 1);
    let d2 = ders.splice(l, 1)[0];
    let d1 = d2.splice(0, p);
    d2.splice(0, 1);
    if (l1.length >= 20) {
        lines.push(l1);
        ders.push(d1);
